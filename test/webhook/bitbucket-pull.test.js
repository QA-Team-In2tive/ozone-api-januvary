...
describe("Bitbucket pull webhook test case", () => {
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.PULL) {
                        ...
                        console.log("Bitbucket webhook repository pull id and name: ", [obj.id, obj.name]);
                        addContext(this, "Bitbucket webhook repository pull id and name: " + [obj.id, obj.name]);
                    }
                }
                done();
            });
    ...
    it("POST /webhooks", function (done) {
        webhook.webhooks(param.bitbucketPull())
            .end((err, res) => {
                if (res.statusCode !== 200)
