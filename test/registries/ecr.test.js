...
describe('Create AWS ECR registry test case', () => {
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.ECR) {
                        ...
                        console.log('ECR registry provider type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'ECR registry provider type id and name: ' + [obj.id, obj.name]);
                    }
                }
    ...
    it('GET /provider', function (done) {
        ...
        providers.get(query.queryParam + provider_type_id)
            .end((err, res) => {
                ...
                console.log('ECR registry provider id and name: ', [id, name]);
                addContext(this, 'ECR registry provider id and name: ' + [id, name]);
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.region_name === constants.AWS_Region) {
                        ...
                        console.log('ECR registry region id and name: ', [obj.id, obj.region_name]);
                        addContext(this, 'ECR registry region id and name: ' + [obj.id, obj.region_name]);
                    }
                }
                done();
            });
    ...
    it('POST /registry', function (done) {
        const param = {
            ...
            'name': 'ecr-' + common.getSmallCharStr(4),
        ...
        registries.create(param)
            .end((err, res) => {
                ...
                console.log('ECR create registry id and name: ', [res.body.id, res.body.name]);
                addContext(this, 'ECR create registry id and name: ' + [res.body.id, res.body.name]);
