...
describe('Link snyk container scan pipeline test case', () => {
            ...
            app.pipeline(query.queryParam)
                .end((err, res) => {
                    ...
                    for (const obj of res.body) {
                        if (obj.name === constants.SNYK_CONTAINER) {
                            elementHandler.toObject(obj, "Snyk response should be object.");
                            elementHandler.toString(obj.id, "Snyk id type should be string.");
                            elementHandler.toString(obj.name, "Snyk name type should be string.");
                            elementHandler.toBeEqual(obj.name, constants.SNYK_CONTAINER);

                            common.readOrWriteJsonFile('app', 'link_pipeline', 'id', obj.id);

                            console.log('Link snyk pipeline id and name: ', [obj.id, obj.name]);
                            addContext(this, 'Link snyk pipeline id and name: ' + [obj.id, obj.name]);
                        }
                    }
                    done();
                });
        ...
        it('POST /app/pipelines', function (done) {
            ...
            app.appPipeline(param)
                .end((err, res) => {
                    ...
                    elementHandler.toObject(res.body, "Snyk response should be object.");
                    elementHandler.toString(res.body.message, "Snyk id type should be string.");

                    console.log('Link snyk pipeline message: ', [msg]);
                    addContext(this, 'Link snyk pipeline message: ' + [msg]);
                    done();
                });
        });
    });
    ...
    describe('Pipeline run details', function () {
        ...
        it('GET /provider', function (done) {
            providers.get(query.queryParam + "&provider_type_id=31")
                .end((err, res) => {
                    if (res.statusCode !== 200)
                        logs.getApiResponse(res);
                    ...
                    common.readOrWriteJsonFile('app', 'link_pipeline', 'provider_id', id);

                    console.log('Pipeline sonarqube provider id and name: ', [id, name]);
                    addContext(this, 'Pipeline sonarqube provider id and name: ' + [id, name]);
                    done();
                });
        });

        it('GET /registry/tags', function (done) {
            const id = common.searchOfEnv('app', 'custom_app', 'registry_id');

            app.tag(query.queryParam + '&registry_id=' + id)
                .end((err, res) => {
                    if (res.statusCode !== 200)
                        logs.getApiResponse(res);
                    ...
                    common.readOrWriteJsonFile('app', 'tag', 'id', id);
                    common.readOrWriteJsonFile('app', 'tag', 'name', name);

                    console.log('Pipeline tag id and name: ', [id, name]);
                    addContext(this, 'Pipeline tag id and name: ' + [id, name]);
                    done();
                });
        });
        ...
        it('POST /app/newdeploy', function (done) {
            ...
            app.newdeploy(param.snykContainer())
                .end((err, res) => {
                    if (res.statusCode !== 200)
...
describe('Link snyk repo scan pipeline test case', () => {
    describe('Pipeline link', () => {
        it('GET /pipelines', function (done) {
            app.pipeline(query.queryParam)
                .end((err, res) => {
                    ...
                    elementHandler.responseExpect(res.body, res.statusCode, 200, res.res.statusMessage, constants.STATUS_TEXT_OK, res.ok);
                    for (const obj of res.body) {
                        if (obj.name === constants.SNYK_REPO) {
                            elementHandler.toObject(obj, "Snyk response should be object.");
                            elementHandler.toString(obj.id, "Snyk id type should be string.");
                            elementHandler.toString(obj.name, "Snyk name type should be string.");
                            elementHandler.toBeEqual(obj.name, constants.SNYK_REPO);
