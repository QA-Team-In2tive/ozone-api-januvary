...
describe('Link canary deploy pipeline test case', () => {
            ...
            app.pipeline(query.queryParam)
                .end((err, res) => {
                    ...
                    for (const obj of res.body) {
                        if (obj.name === constants.CANARY_DEPLOY) {
                            common.readOrWriteJsonFile('app', 'link_pipeline', 'id', obj.id);
                            elementHandler.toBeEqual(obj.name, constants.CANARY_DEPLOY);

                            console.log('Link canary deploy pipeline id and name: ', [obj.id, obj.name]);
                            addContext(this, 'Link canary deploy pipeline id and name: ' + [obj.id, obj.name]);
                        }
                    }
                    done();
                });
        ...
        it('POST /app/pipelines', function (done) {
            ...
            app.appPipeline(param)
                .end((err, res) => {
                    ...
                    console.log('Link canary deploy pipeline message: ', [res.body.message]);
                    addContext(this, 'Link canary deploy pipeline message: ' + [res.body.message]);
                    done();
                });
        });
    });
    ...
    describe('Pipeline run details', function () {
        ...
        it('POST /app/newdeploy', function (done) {
            ...
            app.newdeploy(param.canaryDeploy())
                .end((err, res) => {
                    if (res.statusCode !== 200)
