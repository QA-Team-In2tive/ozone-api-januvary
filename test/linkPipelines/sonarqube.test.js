...
describe('Link sonarqube pipeline test case', () => {
            ...
            app.pipeline(query.queryParam)
                .end((err, res) => {
                    ...
                    for (const obj of res.body) {
                        if (obj.name === constants.SONARQUBE_REPO) {
                            common.readOrWriteJsonFile('app', 'link_pipeline', 'id', obj.id);
                            elementHandler.toBeEqual(obj.name, constants.SONARQUBE_REPO);

                            console.log('Link sonarqube pipeline id and name: ', [obj.id, obj.name]);
                            addContext(this, 'Link sonarqube pipeline id and name: ' + [obj.id, obj.name]);
                        }
                    }
                    done();
                });
        ...
        it('POST /app/pipelines', function (done) {
            ...
            app.appPipeline(param)
                .end((err, res) => {
                    ...
                    console.log('Link sonarqube pipeline message: ', [msg]);
                    addContext(this, 'Link sonarqube pipeline message: ' + [msg]);
                    done();
                });
        });
    });
    ...
    describe('Pipeline run details', function () {
        ...
        it('GET /provider', function (done) {
            providers.get(query.queryParam + "&provider_type_id=32")
                .end((err, res) => {
                    if (res.statusCode !== 200)
                        logs.getApiResponse(res);
