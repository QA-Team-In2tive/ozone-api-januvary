...
const accessType = 'Azure Access Token';

describe('Create Azure DevOps provider test case', () => {
    it('GET /provider/types', function (done) {
        providers.types('?id=7')
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.ADO) {
                        elementHandler.toBeTrue(obj.enabled);
                        elementHandler.toBeEqual(obj.name, constants.ADO);
    ...
    it('POST /provider', function (done) {
        const param = {
            'name': 'azuredevops-' + common.getRandomString(2),
            'organization_name': process.env.azure_devops_org,
            'access_token': process.env.azure_devops_token,
        }
