...
const accessType = 'Credentials';

describe('Create AWS EKS provider test case', () => {
    it('GET / provider/types', function (done) {
        providers.types('?id=13')
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.EKS) {
                        elementHandler.toBeTrue(obj.enabled);
                        elementHandler.toBeEqual(obj.name, constants.EKS);
                        ...
                        console.log('AWS EKS provider type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'AWS EKS provider type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === accessType) {
                        ...
                        console.log('AWS EKS access type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'AWS EKS access type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.region_name === constants.AWS_Region) {
                        ...
                        console.log('AWS EKS provider region id and name: ', [obj.id, obj.region_name]);
                        addContext(this, 'AWS EKS provider region id and name: ' + [obj.id, obj.region_name]);
                    }
                }
                done();
            });
    ...
    it('POST / provider', function (done) {
        const param = {
            'name': 'eks-' + common.getRandomString(2),
        ...
        providers.create()
            ...
            .end((err, res) => {
                ...
                console.log('AWS EKS create provider id and name: ', [res.body.id, res.body.name]);
                addContext(this, 'AWS EKS create provider id and name: ' + [res.body.id, res.body.name]);
                done();
            });
    });
