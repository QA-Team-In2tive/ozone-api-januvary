...
describe('Create Google GKE provider test case', () => {
    it('GET / provider/types', function (done) {
        providers.types('?id=11')
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.GKE) {
                        elementHandler.toBeTrue(obj.enabled);
                        elementHandler.toBeEqual(obj.name, constants.GKE);
                        ...
                        console.log('Google GKE provider type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'Google GKE provider type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === accessType) {
                        ...
                        console.log('Google GKE access type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'Google GKE access type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.region_name === constants.GCP_Region) {
                        ...
                        console.log('Google GKE provider region id and name: ', [obj.id, obj.region_name]);
                        addContext(this, 'Google GKE provider region id and name: ' + [obj.id, obj.region_name]);
                    }
                }
                done();
            });
    ...
    it('POST / provider', function (done) {
        ...
        const param = {
            "name": "gke-" + common.getRandomString(4),
        ...
        providers.create()
            ...
            .end((err, res) => {
                ...
                console.log('Google GKE create provider id and name: ', [res.body.id, res.body.name]);
                addContext(this, 'Google GKE create provider id and name: ' + [res.body.id, res.body.name]);
