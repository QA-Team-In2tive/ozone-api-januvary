...
describe("Create Azure AKS provider test case", () => {
    it("GET / provider/types", function (done) {
        providers.types("?id=9")
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.AKS) {
                        elementHandler.toBeTrue(obj.enabled);
                        elementHandler.toBeEqual(obj.name, constants.AKS);
                        ...
                        console.log("Azure AKS provider type id and name: ", [obj.id, obj.name]);
                        addContext(this, "Azure AKS provider type id and name: " + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === accessType) {
                        ...
                        console.log("Azure AKS provider access type id and name: ", [obj.id, obj.name]);
                        addContext(this, "Azure AKS provider access type id and name: " + [obj.id, obj.name]);
                    }
                }
                done();
            });
    ...
    it("GET / provider/regions", function (done) {
        providers.region("?id=32")
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.region_name === constants.AC_Region) {
                        elementHandler.toBeEqual(obj.region_name, constants.AC_Region);
                        ...
                        console.log("Azure AKS provider region id and name: ", [obj.id, obj.region_name]);
                        addContext(this, "Azure AKS provider region id and name: " + [obj.id, obj.region_name]);
                    }
                }
                done();
            });
    ...
    it("POST / provider", function (done) {
        const param = {
            "name": "aks-" + common.getRandomString(4),
        ...
        providers.create()
            ...
            .end((err, res) => {
                ...
                console.log("Azure AKS provider creation id and name: ", [res.body.id, res.body.name]);
                addContext(this, "Azure AKS provider creation id and name: " + [res.body.id, res.body.name]);
