...
const accessType = "JIRA Token";

describe("Create JIRA provider test case", () => {
    it("GET /provider/types", function (done) {
        providers.types("?id=28")
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.JIRA) {
                        elementHandler.toBeTrue(obj.enabled);
                        elementHandler.toBeEqual(obj.name, constants.JIRA);
                        ...
                        console.log("JIRA provider type id and name: ", [obj.id, obj.name]);
                        addContext(this, "JIRA provider type id and name: " + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === accessType) {
                        ...
                        console.log("JIRA access type id and name: ", [obj.id, obj.name]);
                        addContext(this, "JIRA access type id and name: " + [obj.id, obj.name]);
                    }
                }
                done();
            });
    ...
    it("POST /provider", function (done) {
        const param = {
            "name": "jira-" + common.getRandomString(2),
            "password": process.env.jira_token,
            "url": process.env.jira_url,
            "user_name": process.env.jira_user,
        }
        ...
        providers.create()
            ...
            .end((err, res) => {
                ...
                if (common.isEmpty(res.body)) {
                    ...
                    console.log("JIRA create provider id and name: ", [res.body.id, res.body.name]);
                    addContext(this, "JIRA create provider id and name: " + [res.body.id, res.body.name]);
                }
                done();
            });
    });
