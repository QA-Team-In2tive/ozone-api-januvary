...
describe("Create azure container registry provider test case", () => {
    it("GET / provider/types", function (done) {
        providers.types("?id=8")
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.ACR) {
                        elementHandler.toBeTrue(obj.enabled);
                        elementHandler.toBeEqual(obj.name, constants.ACR);
                        ...
                        console.log("ACR provider type id and name: ", [obj.id, obj.name]);
                        addContext(this, "ACR provider type id and name: " + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === accessType) {
                        ...
                        console.log("ACR provider access type id and name: ", [obj.id, obj.name]);
                        addContext(this, "ACR provider access type id and name: " + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.region_name === constants.AC_Region) {
                        ...
                        console.log("ACR provider region id and name: ", [obj.id, obj.region_name]);
                        addContext(this, "ACR provider region id and name: " + [obj.id, obj.region_name]);
                    }
                }
                done();
            });
    ...
    it("POST / provider", function (done) {
        const param = {
            "name": "acr-" + common.getRandomString(4),
        ...
        providers.create()
            ...
            .end((err, res) => {
                ...
                console.log("ACR provider creation id and name: ", [res.body.id, res.body.name]);
                addContext(this, "ACR provider creation id and name: " + [res.body.id, res.body.name]);
