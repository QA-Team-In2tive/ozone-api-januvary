...
describe('Create harbor provider test case', () => {
    it('GET / provider/types', function (done) {
        providers.types('?id=24')
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.HARBOR) {
                        ...
                        console.log('Harbor provider type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'Harbor provider type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === accessType) {
                        ...
                        console.log('Harbor access type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'Harbor access type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
    ...
    it('POST / provider', function (done) {
        const param = {
            "name": "harbor-" + common.getRandomString(2),
        ...
        providers.create()
            ...
            .end((err, res) => {
                ...
                console.log('Harbor create provider id and name: ', [res.body.id, res.body.name]);
                addContext(this, 'Harbor create provider id and name: ' + [res.body.id, res.body.name]);
