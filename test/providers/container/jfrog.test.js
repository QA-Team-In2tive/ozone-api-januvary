...
const accessType = 'Jfrog Credentials';

describe('Create Jfrog provider test case', () => {
    it('GET /provider/types', function (done) {
        providers.types('?id=22')
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.JFROG) {
                        elementHandler.toBeTrue(obj.enabled);
                        elementHandler.toBeEqual(obj.name, constants.JFROG);
                        ...
                        console.log('Jfrog provider type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'Jfrog provider type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === accessType) {
                        ...
                        console.log('Jfrog access type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'Jfrog access type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
    ...
    it('POST /provider', function (done) {
        const param = {
            "name": "jfrog-" + common.getRandomString(2),
            "password": process.env.jfrog_password,
            "user_name": process.env.jfrog_username,
        ...
        providers.create()
            ...
            .end((err, res) => {
                ...
                console.log('Jfrog create provider id and name: ', [res.body.id, res.body.name]);
                addContext(this, 'Jfrog create provider id and name: ' + [res.body.id, res.body.name]);
