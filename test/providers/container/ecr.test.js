...
describe('Create AWS ECR provider test case', () => {
    it('GET / provider/types', function (done) {
        providers.types('?id=14')
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.ECR) {
                        elementHandler.toBeTrue(obj.enabled);
                        elementHandler.toBeEqual(obj.name, constants.ECR);
                        ...
                        console.log('ECR provider type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'ECR provider type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === accessType) {
                        ...
                        console.log('ECR access type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'ECR access type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.region_name === constants.AWS_Region) {
                        ...
                        console.log('ECR provider region id and name: ', [obj.id, obj.region_name]);
                        addContext(this, 'ECR provider region id and name: ' + [obj.id, obj.region_name]);
                    }
                }
                done();
            });
    ...
    it('POST / provider', function (done) {
        const param = {
            "name": "ecr-" + common.getRandomString(4),
        ...
        providers.create()
            ...
            .end((err, res) => {
                ...
                console.log('ECR create provider id and name: ', [res.body.id, res.body.name]);
                addContext(this, 'ECR create provider id and name: ' + [res.body.id, res.body.name]);
