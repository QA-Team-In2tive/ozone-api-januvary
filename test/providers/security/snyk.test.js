...
const accessType = "SNYK Token";

describe("Create SNYK provider test case", () => {
    it("GET / provider/types", function (done) {
        providers.types("?id=31")
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.SNYK) {
                        elementHandler.toBeTrue(obj.enabled);
                        elementHandler.toBeEqual(obj.name, constants.SNYK);
                        ...
                        console.log("SNYK provider type id and name: ", [obj.id, obj.name]);
                        addContext(this, "SNYK provider type id and name: " + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === accessType) {
                        ...
                        console.log("SNYK access type id and name: ", [obj.id, obj.name]);
                        addContext(this, "SNYK access type id and name: " + [obj.id, obj.name]);
                    }
                }
                done();
            });
    ...
    it("POST / provider", function (done) {
        const param = {
            "name": "snyk-" + common.getRandomString(2),
            "access_token": process.env.snyk_token,
            "url": process.env.snyk_url,
            "self_hosted": true,
        }
        ...
        providers.create()
            ...
            .end((err, res) => {
                ...
                if (common.isEmpty(res.body)) {
                    ...
                    console.log("SNYK create provider id and name: ", [res.body.id, res.body.name]);
                    addContext(this, "SNYK create provider id and name: " + [res.body.id, res.body.name]);
                }
                done();
            });
    });
