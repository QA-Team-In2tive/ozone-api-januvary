...
const accessType = "SONARQUBE TOKEN";

describe("Create SonarQube provider test case", () => {
    it("GET / provider/types", function (done) {
        providers.types("?id=32")
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.SONAR) {
                        elementHandler.toBeTrue(obj.enabled);
                        elementHandler.toBeEqual(obj.name, constants.SONAR);
                        ...
                        console.log("SonarQube provider type id and name: ", [obj.id, obj.name]);
                        addContext(this, "SonarQube provider type id and name: " + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === accessType) {
                        ...
                        console.log("SonarQube access type id and name: ", [obj.id, obj.name]);
                        addContext(this, "SonarQube access type id and name: " + [obj.id, obj.name]);
                    }
                }
                done();
            });
    ...
    it("POST / provider", function (done) {
        const param = {
            "name": "sonar-" + common.getRandomString(2),
            "access_token": process.env.sonar_token,
        ...
        providers.create()
            ...
            .end((err, res) => {
                ...
                if (common.isEmpty(res.body)) {
                    ...
                    console.log("SonarQube create provider id and name: ", [res.body.id, res.body.name]);
                    addContext(this, "SonarQube create provider id and name: " + [res.body.id, res.body.name]);
                }
                done();
            });
    });
