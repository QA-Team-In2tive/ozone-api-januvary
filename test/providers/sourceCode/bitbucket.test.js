...
const accessType = "Bitbucket Token";

describe("Create Bitbucket provider test case", () => {
    it("GET / provider/types", function (done) {
        providers.types("?id=4")
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.BB) {
                        elementHandler.toBeTrue(obj.enabled);
                        elementHandler.toBeEqual(obj.name, constants.BB);
                        ...
                        console.log("Bitbucket provider type id and name: ", [obj.id, obj.name]);
                        addContext(this, "Bitbucket provider type id and name: " + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === accessType) {
                        ...
                        console.log("Bitbucket access type id and name: ", [obj.id, obj.name]);
                        addContext(this, "Bitbucket access type id and name: " + [obj.id, obj.name]);
                    }
                }
                done();
            });
    ...
    it("GET / repo/orgs", function (done) {
        ...
        const query =
            '&password=' + process.env.token_bitbucket +
            '&provider_type_id=' + pro_id +
            '&user_name=' + process.env.user_bitbucket;
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.ORGSBIT) {
                        ...
                        console.log("Bitbucket orgs name and url: ", [obj.name, obj.org_img_url]);
                        addContext(this, "Bitbucket orgs name and url: " + [obj.name, obj.org_img_url]);
                    }
                }
                done();
            });
    ...
    it("POST / provider", function (done) {
        const param = {
            "name": "bit-" + common.getRandomString(2),
            "user_name": process.env.user_bitbucket,
            "password": process.env.token_bitbucket,
        ...
        providers.create()
            ...
            .end((err, res) => {
                ...
                if (common.isEmpty(res.body)) {
                    ...
                    console.log("Bitbucket create provider id and name: ", [res.body.id, res.body.name]);
                    addContext(this, "Bitbucket create provider id and name: " + [res.body.id, res.body.name]);
                }
                done();
            });
    });
