...
describe('Create GCP cloud provider test case', () => {
    it('GET / provider/types', function (done) {
        providers.types('?id=2')
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.GCP) {
                        ...
                        console.log('GCP provider type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'GCP provider type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === accessType) {
                        ...
                        console.log('GCP access type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'GCP access type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.region_name === constants.GCP_Region) {
                        ...
                        console.log('GCP provider region id and name: ', [obj.id, obj.region_name]);
                        addContext(this, 'GCP provider region id and name: ' + [obj.id, obj.region_name]);
                    }
                }
                done();
            });
    ...
    it('POST / provider', function (done) {
        ...
        const param = {
            "name": "gcp-" + common.getRandomString(4),
        ...
        providers.create()
            ...
            .end((err, res) => {
                ...
                console.log('GCP create provider id and name: ', [res.body.id, res.body.name]);
                addContext(this, 'GCP create provider id and name: ' + [res.body.id, res.body.name]);
