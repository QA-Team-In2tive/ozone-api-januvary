...
describe('Create AWS cloud provider test case', () => {
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.AWS) {
                        ...
                        console.log('AWS provider type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'AWS provider type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === accessType) {
                        ...
                        console.log('AWS access type id and name: ', [obj.id, obj.name]);
                        addContext(this, 'AWS access type id and name: ' + [obj.id, obj.name]);
                    }
                }
                done();
            });
            ...
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.region_name === constants.AWS_Region) {
                        ...
                        console.log('AWS provider region id and name: ', [obj.id, obj.region_name]);
                        addContext(this, 'AWS provider region id and name: ' + [obj.id, obj.region_name]);
                    }
                }
                done();
            });
    ...
    it('POST / provider', function (done) {
        const param = {
            'name': 'aws-' + common.getRandomString(4),
        ...
        providers.create()
            ...
            .end((err, res) => {
                ...
                console.log('AWS create provider id and name: ', [res.body.id, res.body.name]);
                addContext(this, 'AWS create provider id and name: ' + [res.body.id, res.body.name]);
                done();
            });
    });
