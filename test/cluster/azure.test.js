...
describe('Create Azure cloud cluster test case', () => {
    it('List / provider/types', function (done) {
        providers.types(query.types + '&sub_category_id=5&sub_category_id=3')
            .end((err, res) => {
                if (res.statusCode !== 200)
                    logs.getApiResponse(res);
    ...
    it('List / provider/nodetype', function (done) {
        ...
        clusters.nodeType(query.queryParam + ids)
            .end((err, res) => {
                ...
                for (const obj of res.body) {
                    if (obj.name === constants.NODE_TYPE_ACZ) {
                        elementHandler.toBeEqual(obj.name, constants.NODE_TYPE_ACZ);
    ...
    it('POST / cluster', function (done) {
        clusters.create(param.clusterAzureCreation())
            .end((err, res) => {
                if (res.statusCode !== 200)
                ...
                console.log('Azure cloud cluster creation id: ', res.body.clusterID);
                addContext(this, 'Azure cloud cluster creation id: ' + res.body.clusterID);
                done();
            });
    });
            ...
            await clusters.get('/' + id[0].replace(/["']/g, ""))
                .then((res) => {
                    ...
                    if (value) {
                        ...
                        common.readOrWriteJsonFile('cluster', 'cluster_details', 'name', resp.name)
                    } else {
                        count += 1;
